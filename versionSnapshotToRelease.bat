...
if "%CURRENT_VERSION%"=="" (
    echo can not extract project.version from pom
    exit /b 1
)
...
if "%LAST9%"=="-SNAPSHOT" (
    set NEW_VERSION=%CURRENT_VERSION:~0,-9%
) else (
    echo ERROR: current version is already release
    exit /b 1
)

echo new version: %NEW_VERSION%

REM 3) set new version string
