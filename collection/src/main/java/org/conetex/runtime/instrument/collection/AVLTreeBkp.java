...
public class AVLTreeBkp<T extends Comparable<T>> {
    ...
    private static class Node<D extends Comparable<D>> implements Entry<D> {
        ...
        private Node<D> insert(D valueToInsert) {
            ...
            if (cmp < 0){
                ...
                Node<D> newLeft = oldLeft.insert(valueToInsert);
                synchronized(this) {
                    if ( newLeft.height - 1 > (oldRight == null ? 0 : oldRight.height) ) {
                        // left heavy
                        if ( newLeft.right == null || (newLeft.left != null && newLeft.left.height >= newLeft.right.height) ) {
                            // LL
                            //return newLeft.rootRotateRight(this);
                            return this.rotateRightNew(newLeft, oldLeft, oldRight);
                        }
                        // left.right HIGHER ==> LR
                        //return newLeft.right.rootRotateLeft(newLeft).rootRotateRight( this );
                        return this.rotateLeftRightNew(newLeft, oldLeft, oldRight);
                    }
        ...
        // LL
        private Node<D> rotateRightNew(Node<D> newRoot, Node<D> oldLeft, Node<D> oldRight) {

            // Perform rotation
            Node<D> newThis = new Node<>(this.data, newRoot.right, oldRight);
        ...
        // RR
        private Node<D> rotateLeftNew(Node<D> newRoot, Node<D> oldLeft, Node<D> oldRight) {

            // Perform rotation
            Node<D> newThis = new Node<>(this.data, oldLeft, newRoot.left);
        ...
        // LR
        private Node<D> rotateLeftRightNew(Node<D> newLeft, Node<D> oldLeft, Node<D> oldRight) {
            if(newLeft == oldLeft){
                // newLeft is not new ==> make it new
                return new Node<>(
                        newLeft.right.data,
                        new Node<>(newLeft.data, newLeft.left, newLeft.right.left),
                        new Node<>(this.data, newLeft.right.right, oldRight)
                );
            }
            else{
        ...
        // RL
        private Node<D> rotateRightLeftNew(Node<D> newRight, Node<D> oldLeft, Node<D> oldRight) {
            if(newRight == oldRight){
                // newRight is not new ==> make it new
                return new Node<D>(
                        newRight.left.data,
                        new Node<D>(this.data, oldLeft, newRight.left.left),
        ...
        private Node<D> setRightUpdateHeight(Node<D> newRight) {
            //synchronized (this.monitorRight){
                this.right = newRight;
                this.updateHeight();
                return this;
        ...
        private Node<D> setLeftUpdateHeight(Node<D> newLeft) {
            //synchronized (this.monitorLeft){
                this.left = newLeft;
                this.updateHeight();
                return this;
    ...
    public static void main(String[] args) {
        AVLTreeBkp<Integer> tree = new AVLTreeBkp<>();
